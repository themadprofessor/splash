...
use splash_rs::endpoint::photos::Photo;
use splash_rs::error::Error as SplashError;

fn main() {
    if let Err(e) = run() {
        eprintln!("{}{}", e, e.iter_causes()
            .map(|err| format!("\n\tcause: {}", err))
            .fold(String::new(), |mut s, err| {
                s += &err;
                s
            }));
        ::std::process::exit(-1);
...
fn run() -> Result<(), Error> {
    ...
    if let Some(query) = matches.value_of_lossy("query") {
        let fut = fut.query(query.to_string())
        ...
        let fut = fut.collection(collections)
            .get(&client, "87e5c4f5e3db3a47a9cbc9abefbd196e3f7aa9a7cccc1ca4751008ec796e4eb7")
            .map_err(Error::from)
            .and_then(|p| match wallpaper::set_from_url(&p.urls.full) {
                Ok(_) => ::futures::future::ok(()),
        ...
        let fut = fut.get(&client, "87e5c4f5e3db3a47a9cbc9abefbd196e3f7aa9a7cccc1ca4751008ec796e4eb7")
            .map_err(Error::from)
            .and_then(|p| match wallpaper::set_from_url(&p.urls.full) {
                Ok(_) => ::futures::future::ok(()),
                Err(e) => ::futures::future::err(::failure::err_msg(format!("{}", e)))
...
fn build_cli() -> App<'static, 'static> {
    (app_from_crate!() as App)
        ...
        .arg(Arg::with_name("collection")
            ...
            .long_help("Restrict images to only images which are in any of the given collections")
